...
class LVC:
    def __init__(self, device='auto', use_xl_model=True):
        if device == 'auto':
    ...
    def infer_file(self, input, sample, output):
        source_audio = load_and_resample(input, self.hp.audio.sampling_rate)
        target_audio = load_and_resample(sample, self.hp.audio.sampling_rate)
        ...
        wavfile.write(output, self.hp.audio.sampling_rate, vc_audio)
    def infer_array(self, input_audio: LVCAudio, sample_audio: LVCAudio):
        new_sr = self.hp.audio.sampling_rate
        inp_y = input_audio.y
